...
export class UsersComponent implements OnInit {
  ...
  onEditConfirm(event): void {
    console.log('OnEdit');
    if (!this.validate(event.newData, true)) return;
    ...
    this.service.editUser(event.newData).subscribe(
      (data: any) => {
        ...
        event.newData.password = '';
        event.newData.confirmPassword = '';
        this.users = this.users.filter(item=>item._id !== event.newData._id);
        this.users = [event.newData, ...this.users];
        this.source.load(this.users);
      },
      (error) => {
  ...
  validate(user, isEdit = false): boolean {
    ...
    if (isEdit) {
      if (!user.username) {
        this.flashMessagesService.show(
          `Полt логин должно быть заполнено`,
          { cssClass: 'alert-danger', timeout: 7000 }
        );
        return false;
      }
    ...
    if (!user.username.match(/^[a-zA-Zа-яА-Я1-9]*$/)) {
      this.flashMessagesService.show(`Поле логин должно содержать только буквы и цифры `, {
        cssClass: 'alert-danger',
        timeout: 7000,
      });
      return false;
